...
from poker_ai.clustering.card_combos import CardCombosMP
from poker_ai.clustering.game_utility import GameUtility
from poker_ai.clustering.preflop import compute_preflop_lossless_abstraction_2_to_A

log = logging.getLogger("poker_ai.clustering.runner")
# 把所有的可能的牌面组合 (起手，翻牌，转牌，河牌) 按照 牌力/长得像不像 分成一堆簇，然后做成一个查找表LUT，后面打牌的时候 就不用一首一首算仿真，直接查表

class CardInfoLutBuilder(CardCombosMP): #有了所有牌面组合的基础上 再做一步：给这些组合分桶
    """
    ...
    def compute(
        ...
        if "pre_flop" not in self.card_info_lut:
            self.card_info_lut["pre_flop"] = compute_preflop_lossless_abstraction_2_to_A(
                builder=self
            )
