...
@train.command()
@click.option(
    "--strategy_interval",   #每隔多少次 迭代 更新一次策略呢？
    default=20,
...
@click.option(
    "--lcfr_threshold",   #lcfr(linear) 算法的阈值  决定 哪个 iterations 开始 应用折扣   这个决定什么时候开始应用折扣
    default=400,
    help=(
...
@click.option(
    "--discount_interval", # 每隔多少次迭代对当前的遗憾和策略进行折扣  这个决定 折扣的频率是多久
    default=400,
    help=(
...
@click.option(
    "--prune_threshold",  #在此阈值之后 使用带有 剪枝的 cfr 算法
    default=400,
    help=(
...
@click.option("--n_players", default=3, help="The number of players in the game.") #多少人 玩这个游戏
@click.option(
    "--dump_iteration",   #保存间隔
    default=20,
    help=(
...
@click.option(
    "--update_threshold", #迭代次数 超过 这个阈值之后 开始更新策略
    default=400,
    help=(
